...
2. Follow the structure recommended by IEEE 29148 for that chapter (see details table below); remove any empty sub-sections.
...
CATEGORY_SRS = ["1. Introduction", "2. Overall Description", "3. Functional Requirements", "4. Non-functional Requirements", "5. Constraints", "6. Usage Scenarios", "7. Glossary", "8. Requirements Model"]
...
class Archivist(BaseAgent):
    ...
    """讲系统需求整理成SRS需求文档的格式，每次让大模型生成一个章节，最后拼凑成一个SRS需求文档，再把需求模型的plantUML放到最后"""
    def write_srs(self) -> str:
        system_requirements = self.get_memory("system_requirements")
        srs_content = []
        for category in self.category_srs[:-1]:
            srs_content.append(self._generate_response(WRITE_SRS_PROMPT.format(system_requirements=system_requirements, category=category)))
        srs_content = "\n".join(srs_content)
        return srs_content

    def update_srs(self) -> str:
        srs_draft = self.get_memory("srs_draft")
        review_report = self.get_memory("review_report")
        srs_revised = self._generate_response(UPDATE_SRS_PROMPT.format(srs_draft=srs_draft, review_report=review_report))
        self.add_to_memory("srs_revised", srs_revised)
        return srs_revised
