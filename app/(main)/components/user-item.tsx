        ...
        <div role="button" className="flex w-full items-center p-3 text-sm hover:bg-primary/5">
          <div className="flex max-w-[150px] items-center gap-x-2">
            <Avatar className="h-5 w-5">
              <AvatarImage src={user?.image || ''} alt={user?.name || 'anonymous'}></AvatarImage>
            </Avatar>
            <span className="line-clamp-1 text-start font-medium">{user?.name}&apos;s Jotlin</span>
          </div>
        ...
        <div className="flex flex-col space-y-4 p-2">
          ...
          <div className="flex items-center gap-x-2">
            ...
            <div className="space-y-1">
              <p className="line-clamp-1 text-sm">{user?.name || 'anonymous'}&apos;s Jotlin</p>
            </div>
          </div>
        </div>
        <DropdownMenuSeparator />
