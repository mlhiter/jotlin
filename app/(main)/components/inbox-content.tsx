...
const InboxContent = () => {
  ...
  const { invitations, fetchInvitations, markInboxAsViewed } = useInvitationStore()
  ...
  if (invitations.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4 p-4">
        ...
        <Button variant="outline" size="sm" onClick={handleRefresh} disabled={isRefreshing} className="h-8 px-3">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="text-sm font-medium">Invitations ({invitations.length})</div>
      ...
      <ScrollArea className="h-[350px]">
        {invitations.map((invitation, index) => (
          <div key={invitation.id} className={index < invitations.length - 1 ? 'mb-3' : ''}>
